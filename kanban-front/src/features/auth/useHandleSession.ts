...
import {
  ...
  LOGIN_GUEST,
  REGISTER,
} from "@/GraphQL Queries/SessionQueries.ts";
import { useAppDispatch, useAppSelector } from "@/app/hooks.ts";
import { setAuth } from "@/features/auth/AuthenticationSlice.tsx";
import {
  notifyError,
  notifySuccess,
} from "@/generalComponents/toastService.ts";
...
function useHandleSession({ setProfileShow }: PropTypes = {}) {
  ...
  const handleRegister = async () => {
    try {
      ...
      if (data.successful && data.userId) {
        ...
        notifySuccess(
          `Hi ${name}, you are provided with initial data to test the functionality, feel free to delete it and create your own!`,
          8000
        );
        ...
        if (data.existingEmail) {
          setUsedEmail(
            "this email is already associated with an account. Please try another one."
          );
        }
        if (data.existingUser) {
          setUsedName(
            "this user name is already associated with an account. Please try another one."
          );
        }
      }
    } catch (err) {
  ...
  const handleGuest = async () => {
    ...
    if (id) {
      notifySuccess(
        "Hello There! You can now start using the app, the changes you make will be lost once logged out or after a period of two hours, to persist them register a new account!",
        10000
      );
  ...
  const generalLoad =
    loading || registeringLoad || loggingGuest || changeLoading;
  return {
    handleSubmit,
    name,
    setName,
